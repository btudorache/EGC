...
void main()
{
    vec3 floorColors = vec3(0, 0, 0);
    for (int i = 0; i < 9; i++) {
        floorColors += PointLightContribution(pointLightPos[i], pointLightColor[i]);
    }

    vec3 color = object_color * floorColors + 0.1 * DiscoLightContribution() + ConeLightContribution();
...
vec3 PointLightContribution(vec3 lightPos, vec3 lightColor)
{
	...
	vec3 color = lightColor * mitigationFactor * (20 * diffuse_light + 30 * specular_light);
